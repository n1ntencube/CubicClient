(async () => {
    ...
    try {
      ...
      if (res && res.ok) {
        ...
        if (aboutRepoEl && pkg.repository) {
          const repo = typeof pkg.repository === 'string' ? pkg.repository : (typeof pkg.repository === 'object' && pkg.repository.url ? pkg.repository.url : pkg.repository)
  ...
  const modShopBtn = document.getElementById('modShopBtn')
  if (modShopBtn) {
    modShopBtn.addEventListener('click', () => {
    ...
    launchBtn.addEventListener('click', async () => {
      try {
        ...
        try {
          ...
          if (modListRes && modListRes.ok && Array.isArray(modListRes.mods)) {
            const mandatoryMods = modListRes.mods.filter(m => (m && (m.mandatory === true || m.mandatory === 1)))
            modsUrls = mandatoryMods.map(mod => {
              if (typeof mod === 'string') return mod
              if (mod && mod.url) return mod.url
              return null
            }).filter(Boolean)
            console.log(`[Launch] Found ${modsUrls.length} mandatory mods to install`)
          } else {
